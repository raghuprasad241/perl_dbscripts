...
foreach my $r1 (0 .. $#row)
{
    ...
    my @remain = grep !/$r1/,(0 .. $#row); # @remain array contains all the rows other than the current row of @row for comparision
    foreach my $r2 (@remain) 
      {
       ...
       foreach my $c1 (0..$#{$row[$r1]})
          {
                 foreach my $c2 (0..$#{$row[$r2]})
                 {
                    ...
                    if ($c1==$c2) # Make sure we are swapping values on the same column not across the columns
                       {
